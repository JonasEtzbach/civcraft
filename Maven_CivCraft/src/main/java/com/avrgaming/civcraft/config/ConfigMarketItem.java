...
public class ConfigMarketItem {
	...
	public static void init() throws SQLException {
		if (!SQL.hasTable(TABLE_NAME)) {
			String table_create = "CREATE TABLE " + SQL.tb_prefix + TABLE_NAME + " (" + "`ident` VARCHAR(64) NOT NULL,"
					+ "`buy_value` int(11) NOT NULL DEFAULT 105," + "`buy_bulk` int(11) NOT NULL DEFAULT 1,"
					+ "`sell_value` int(11) NOT NULL DEFAULT 95," + "`sell_bulk` int(11) NOT NULL DEFAULT 1,"
					+ "`buysell` int(11) NOT NULL DEFAULT 0," + "`bought` int(11) NOT NULL DEFAULT 0,"
					+ "`sold` int(11) NOT NULL DEFAULT 0," + "`last_action` mediumtext, " + "PRIMARY KEY (`ident`)"
					+ ")";
	...
	public void saveItemNow() throws SQLException {
		...
		try {
			String query = "INSERT INTO `" + SQL.tb_prefix + TABLE_NAME
					+ "` (`ident`, `buy_value`, `buy_bulk`, `sell_value`, `sell_bulk`, `bought`, `sold`, `last_action`, `buysell`) "
	...
	public void buy(Resident resident, Player player, int amount) throws CivException {
		...
		CivMessage.sendSuccess(player, CivSettings.localize.localizedString("var_market_buy", total_items, this.name,
				cost, CivSettings.CURRENCY_NAME));
	...
	public void sell(Resident resident, Player player, int amount) throws CivException {
		...
		if (!inv.contains(custom_id, this.type_id, (short) this.data, amount)) {
			throw new CivException(
					CivSettings.localize.localizedString("var_market_sell_notEnough", amount, this.name));
		}
		...
		CivMessage.sendSuccess(player, CivSettings.localize.localizedString("var_market_sell", total_items, this.name,
				total_coins, CivSettings.CURRENCY_NAME));
		player.updateInventory();
	}
