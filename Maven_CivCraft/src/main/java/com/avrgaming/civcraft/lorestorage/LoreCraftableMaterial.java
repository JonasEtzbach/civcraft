...
//	public LinkedList<ItemStack> shaplessIngredientList = new LinkedList<ItemStack>();
	// public Map<Character, ItemStack> shapedIngredientList = new
	 ...
	 * on the results. The key is the material's ID as a string, so we can are only
	 * checking for custom items. The Itemstack array is the matrix for the recipe
	 * where the first 3 items represent the top row, and the last 3 represent the
	 * bottom row.
	 */
	...
	/*
	 * We will allow duplicate shaped recipes by checking this map based on the
	 * results. In order for the recipe to be valid it must contain all of the item
	...
	public static void buildStaticMaterials() {
		...
		for (ConfigMaterial cfgMat : CivSettings.materials.values()) {
			LoreCraftableMaterial loreMat = new LoreCraftableMaterial(cfgMat.id, cfgMat.item_id,
					(short) cfgMat.item_data);
			...
			if (loreMat.isShaped()) {
				...
				for (ConfigIngredient ingred : configMaterial.ingredients.values()) {
					...
					if (ingred.custom_id == null) {
						recipe.setIngredient(ingred.letter.charAt(0),
								ItemManager.getMaterialData(ingred.type_id, ingred.data));
						...
						if (customMat != null) {
							recipe.setIngredient(ingred.letter.charAt(0),
									ItemManager.getMaterialData(customMat.item_id, customMat.item_data));
						} else {
							CivLog.warning("Couldn't find custom material id:" + ingred.custom_id);
						}
				...
				for (ConfigIngredient ingred : configMaterial.ingredients.values()) {
					...
					try {
						if (ingred.custom_id == null) {
							recipe.addIngredient(ingred.count,
									ItemManager.getMaterialData(ingred.type_id, ingred.data));
							ingredStack = ItemManager.createItemStack(ingred.type_id, 1, (short) ingred.data);
						} else {
							LoreCraftableMaterial customLoreMat = materials.get(ingred.custom_id);
							if (customLoreMat == null) {
								CivLog.error("Couldn't configure ingredient:" + ingred.custom_id + " in config mat:"
										+ configMaterial.id);
							}
							ConfigMaterial customMat = customLoreMat.configMaterial;
							if (customMat != null) {
						...
						CivLog.warning("Trying to process ingredient:" + ingred.type_id + ":" + ingred.custom_id
								+ " for material:" + configMaterial.id);
						throw e;
					}
...
//		}
	...
	@Override
	public void onInvItemSwap(InventoryClickEvent event, Inventory toInv, ItemStack droppedStack,
