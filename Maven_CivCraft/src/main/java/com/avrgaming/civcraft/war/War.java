...
public class War {
	...
	/*
	 * Rebuild the defeated civs session db vars when a transfer of defeated towns
	 * occurs.
	 */
	...
	public static void setWarTime(boolean warTime) {
		...
		if (warTime == false) {
			...
			CivMessage.globalTitle(
					CivColor.Yellow + CivColor.BOLD + CivSettings.localize.localizedString("war_wartimeEndedHeading"),
					CivSettings.localize.localizedString("var_war_mostLethal", WarStats.getTopKiller()));
			/* display some stats. */
			...
			CivMessage.globalTitle(
					CivColor.Red + CivColor.BOLD + CivSettings.localize.localizedString("war_wartimeBeginHeading"),
					CivSettings.localize.localizedString("war_wartimeBegin_title_length", mins / 60.0));
	...
	/*
	 * When a civ conqueres a town, then has its capitol conquered, the town that it
	 * just conquered needs to go to the new owner. If the new owner was the town's
	 * old owner, then that town is no longer defeated.
	 */
		...
		if (!CivGlobal.isCasualMode()) {
			for (String townName : defeatedTowns.keySet()) {
				try {
					...
					CivMessage.sendTown(town, CivColor.LightBlue
							+ CivSettings.localize.localizedString("var_war_overlordAnnounce", winner.getName()));
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
			...
			for (String civName : defeatedCivs.keySet()) {
				try {
					...
					if (civ == null) {
						/*
						 * Civ is no longer on the list of active civs. Which means it's already been
						 * conquered?
						 */
						CivLog.error("Couldn't find civilization named " + civName
								+ " in civ hash table while trying to process it's defeat.");
						continue;
					}

					Civilization winner = defeatedCivs.get(civName);
					CivMessage.sendCiv(civ, CivColor.LightBlue + CivColor.LightBlue
							+ CivSettings.localize.localizedString("var_war_overlordAnnounce", winner.getName()));
					civ.onDefeat(winner);
				} catch (Exception e) {
					e.printStackTrace();
	...
	/*
	 * The 'claimed' flag is used to tag towns that had illegal town hall
	 * placements, and have already been claimed. When war time starts, we should
	 * reset the claim flag so it can be claimed by someone else.
	 */
