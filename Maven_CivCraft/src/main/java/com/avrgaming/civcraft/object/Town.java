	...
	public static void init() throws SQLException {
		if (!SQL.hasTable(TABLE_NAME)) {
			String table_create = "CREATE TABLE " + SQL.tb_prefix + TABLE_NAME + " ("
					...
					+ "`civ_id` int(11) NOT NULL DEFAULT 0," + "`master_civ_id` int(11) NOT NULL DEFAULT 0," + // XXX no
																												// longer
																												// used.
					"`mother_civ_id` int(11) NOT NULL DEFAULT 0," + "`defaultGroupName` mediumtext DEFAULT NULL,"
					+ "`mayorGroupName` mediumtext DEFAULT NULL," + "`assistantGroupName` mediumtext DEFAULT NULL,"
					+ "`upgrades` mediumtext DEFAULT NULL," + "`level` int(11) DEFAULT 1," + "`debt` double DEFAULT 0,"
					+ "`coins` double DEFAULT 0," + "`daysInDebt` int(11) DEFAULT 0,"
					+ "`flat_tax` double NOT NULL DEFAULT '0'," + "`tax_rate` double DEFAULT 0,"
					+ "`extra_hammers` double DEFAULT 0," + "`culture` int(11) DEFAULT 0," + "`created_date` long,"
					+ "`outlaws` mediumtext DEFAULT NULL," + "`dbg_civ_name` mediumtext DEFAULT NULL,"
...
//			this.happyCoinRate.registerComponent();
	...
	public void addResident(Resident res) throws AlreadyRegisteredException {
		...
		if (player != null && CivSettings.hasITag) {
			// TODO test if it causes a bug change from
	...
	public static Town newTown(Resident resident, String name, Civilization civ, boolean free, boolean capitol,
		...
		try {
			...
			if (resident.getTown() != null && resident.getTown().isMayor(resident)) {
				throw new CivException(CivSettings.localize.localizedString("var_town_found_errorIsMayor",
			...
			for (Town town : CivGlobal.getTowns()) {
				...
				if (dist < minDistance) {
					...
					throw new CivException(CivSettings.localize.localizedString("var_town_found_errorTooClose",
							town.getName(), df.format(dist), minDistance));
				}
			}
			...
			try {
				...
				for (TownChunk cc : CivGlobal.getTownChunks()) {
					...
					if (dist <= min_distance) {
						DecimalFormat df = new DecimalFormat();
						throw new CivException(CivSettings.localize.localizedString("var_town_found_errorTooClose",
								cc.getTown().getName(), df.format(dist), min_distance));
					}
				}
...
//		}
		...
		if (struct.getLimit() != 0) {
			if (getStructureTypeCount(id) >= struct.getLimit()) {
				throw new CivException(CivSettings.localize.localizedString("var_town_structure_errorLimitMet",
						struct.getLimit(), struct.getDisplayName()));
			}
		}
		...
		try {
			/*
			 * XXX if the template is null we need to just get the template first. This
			 * should only happen for capitols and town halls since we need to Make them use
			 * the structure preview code and they don't yet
			 */
			if (tpl == null) {
				try {
					tpl = new Template();
		...
		// TODO fix this dependency nightmare! (the center is moved in build and needs
		// to be resaved)
	...
	public boolean processSpyExposure(Resident resident) {
		...
		try {
			...
			if (percent >= CivSettings.getDouble(CivSettings.espionageConfig, "espionage.town_exposure_failure")) {
				failed = true;
				CivMessage.sendTown(this,
						CivColor.Yellow + CivColor.BOLD + CivSettings.localize.localizedString("town_spy_thwarted"));
				return failed;
			}
