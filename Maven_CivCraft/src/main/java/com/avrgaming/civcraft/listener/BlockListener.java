	...
	public void onProjectileHitEvent(ProjectileHitEvent event) {
		...
		if (event.getEntity() instanceof Fireball) {
			...
			if (cfc != null) {
				...
				FireworkEffect fe = FireworkEffect.builder().withColor(Color.RED).withColor(Color.BLACK).flicker(true)
...
//			CivLog.debug("onEntityDamageByEntityEvent LightningStrike: "+event.getDamager().getUniqueId());
	...
	private final BlockFace[] faces = new BlockFace[] { BlockFace.DOWN, BlockFace.NORTH, BlockFace.EAST,
			BlockFace.SOUTH, BlockFace.WEST, BlockFace.SELF, BlockFace.UP };

	public BlockCoord generatesCobble(int id, Block b) {
		int mirrorID1 = (id == CivData.WATER_RUNNING || id == CivData.WATER ? CivData.LAVA_RUNNING
				: CivData.WATER_RUNNING);
...
//	            	}
	...
	public void OnBlockPlaceEvent(BlockPlaceEvent event) {
		...
		if (sb != null) {
			event.setCancelled(true);
			CivMessage.sendError(event.getPlayer(), CivSettings.localize.localizedString("blockBreak_errorStructure")
			...
			if (rb.isAboveRoadBlock()) {
				if (resident.getCiv() != rb.getRoad().getCiv()) {
					...
					CivMessage.sendError(event.getPlayer(),
							CivSettings.localize.localizedString("blockPlace_errorRoad1") + " " + (Road.HEIGHT - 1)
		...
		if (cb != null && !cb.canBreak(event.getPlayer().getName())) {
			event.setCancelled(true);
			CivMessage.sendError(event.getPlayer(),
					CivSettings.localize.localizedString("blockBreak_errorCamp1") + " " + cb.getCamp().getName() + " "
				...
				if (War.isWarTime() && resident.hasTown()
					...
					if (WarRegen.canPlaceThisBlock(event.getBlock())) {
						...
						CivMessage.sendErrorNoRepeat(event.getPlayer(),
								CivSettings.localize.localizedString("blockPlace_errorWar"));
						event.setCancelled(true);
						return;
					}
				} else {
					event.setCancelled(true);
					CivMessage.sendError(event.getPlayer(),
							CivSettings.localize.localizedString("blockPlace_errorPermission") + " "
									+ tc.getTown().getName());
				}
			}
		}
		 ...
		 */
		// LinkedList<StructureBlock> sbList =
		// CivGlobal.getStructureBlocksAt(bcoord.getWorldname(), bcoord.getX(),
		// bcoord.getZ());
		...
		if (buildables != null) {
			for (Buildable buildable : buildables) {
				...
				/*
				 * Building is validated, grab the layer and determine if this would set it over
				 * the limit.
				 */
	...
	public void OnBlockBreakEvent(BlockBreakEvent event) {
		...
		if (sb != null) {
			event.setCancelled(true);
			TaskMaster.syncTask(
					new StructureBlockHitEvent(event.getPlayer().getName(), bcoord, sb, event.getBlock().getWorld()),
					0);
			return;
		}
		...
		if (structSign != null && !resident.isSBPermOverride()) {
			event.setCancelled(true);
			CivMessage.sendError(event.getPlayer(),
					CivSettings.localize.localizedString("blockBreak_errorStructureSign"));
			return;
		}
		...
		if (structChest != null && !resident.isSBPermOverride()) {
			event.setCancelled(true);
			CivMessage.sendError(event.getPlayer(),
					CivSettings.localize.localizedString("blockBreak_errorStructureChests"));
			return;
		}
			...
			for (Wall wall : walls) {
				if (wall.isProtectedLocation(event.getBlock().getLocation())) {
					if (resident == null || !resident.hasTown()
							|| resident.getTown().getCiv() != wall.getTown().getCiv() && !resident.isSBPermOverride()) {

						StructureBlock tmpStructureBlock = new StructureBlock(bcoord, wall);
						tmpStructureBlock.setAlwaysDamage(true);
						TaskMaster.syncTask(new StructureBlockHitEvent(event.getPlayer().getName(), bcoord,
								tmpStructureBlock, event.getBlock().getWorld()), 0);
						// CivMessage.sendError(event.getPlayer(), "Cannot destroy this block, protected
						// by a wall, destroy it first.");
						event.setCancelled(true);
						return;
					} else {
						CivMessage.send(event.getPlayer(),
								CivColor.LightGray + CivSettings.localize.localizedString("blockBreak_wallAlert") + " "
		...
		if (tc != null) {
			if (!tc.perms.hasPermission(PlotPermissions.Type.DESTROY, resident)) {
				...
				if (War.isWarTime() && resident.hasTown()
					...
					CivMessage.sendErrorNoRepeat(event.getPlayer(),
							CivSettings.localize.localizedString("blockBreak_errorPermission") + " "
									+ tc.getTown().getName());
				}
			}
		}
		...
		if (buildables != null) {
			for (Buildable buildable : buildables) {
				...
				if (percentValid < Buildable.validPercentRequirement) {
					CivMessage.sendError(event.getPlayer(),
							CivSettings.localize.localizedString("blockBreak_errorSupport") + " "
	...
	public void OnEntityInteractEvent(EntityInteractEvent event) {
		if (event.getBlock() != null) {
			if (CivSettings.switchItems.contains(event.getBlock().getType())) {
				...
				if (event.getEntity() instanceof Player) {
					CivMessage.sendErrorNoRepeat((Player) event.getEntity(),
							CivSettings.localize.localizedString("blockUse_errorPermission"));
				}
	...
	public void OnPlayerConsumeEvent(PlayerItemConsumeEvent event) {
		...
		if (ItemManager.getId(event.getItem()) == ItemManager.getId(Material.GOLDEN_APPLE)) {
			if (event.getItem().getDurability() == (short) 0x1) {
				CivMessage.sendError(event.getPlayer(),
						CivSettings.localize.localizedString("itemUse_errorGoldenApple"));
				event.setCancelled(true);
				return;
			}
		}
		...
		if (stack.getType().equals(Material.POTION)) {
			...
			if (effect == 0xE) {
				event.setCancelled(true);
				CivMessage.sendError(event.getPlayer(),
						CivSettings.localize.localizedString("itemUse_errorInvisPotion"));
				return;
			}
		}
	}
	...
	public void OnPlayerInteractEvent(PlayerInteractEvent event) {
		...
		if (event.hasItem()) {
			...
			if (event.getItem().getType().equals(Material.INK_SACK) && event.getItem().getDurability() == 15) {
				...
				if (ItemManager.getId(clickedBlock) == CivData.WHEAT
					...
					event.setCancelled(true);
					CivMessage.sendError(event.getPlayer(),
							CivSettings.localize.localizedString("itemUse_errorBoneMeal"));
					return;
				}
			}
		}
		...
		/*
		 * Right clicking causes some dupe bugs for some reason with items that have
		 * "actions" such as swords. It also causes block place events on top of signs.
		 * So we'll just only allow signs to work with left click.
		 */
		boolean leftClick = event.getAction().equals(Action.LEFT_CLICK_AIR)
		...
		if (event.getClickedBlock() != null) {
			...
			if (MarkerPlacementManager.isPlayerInPlacementMode(event.getPlayer())) {
				...
				try {
					...
					CivMessage.send(event.getPlayer(),
							CivColor.LightGreen + CivSettings.localize.localizedString("itemUse_marked"));
				} catch (CivException e) {
	...
	public static void OnPlayerSwitchEvent(PlayerInteractEvent event) {
		...
		if (cb != null && !resident.isPermOverride()) {
			if (!cb.getCamp().hasMember(resident.getName())) {
				CivMessage.sendError(event.getPlayer(),
						CivSettings.localize.localizedString("blockUse_errorNotInCamp"));
				event.setCancelled(true);
				return;
			}
		}
		...
		if (!tc.perms.hasPermission(PlotPermissions.Type.INTERACT, resident)) {
			...
			if (War.isWarTime() && resident.hasTown()
				...
				CivMessage.sendErrorNoRepeat(event.getPlayer(),
						CivSettings.localize.localizedString("blockUse_errorGeneric") + " "
	...
	private void OnPlayerUseItem(PlayerInteractEvent event) {
		Location loc = (event.getClickedBlock() == null) ? event.getPlayer().getLocation()
		...
		if (camp != null) {
			if (!camp.hasMember(event.getPlayer().getName())) {
				CivMessage.sendError(event.getPlayer(),
						CivSettings.localize.localizedString("itemUse_errorCamp") + " " + stack.getType().toString());
				event.setCancelled(true);
				return;
			}
		}
		...
		if (!tc.perms.hasPermission(PlotPermissions.Type.ITEMUSE, resident)) {
			event.setCancelled(true);
			CivMessage.sendErrorNoRepeat(event.getPlayer(), CivSettings.localize.localizedString("itemUse_errorGeneric")
	...
	public void OnPlayerInteractEntityEvent(PlayerInteractEntityEvent event) {
		...
		if (inHand != null) {
			...
			case HORSE:
				if (inHand.getType().equals(Material.GOLDEN_APPLE) || inHand.getType().equals(Material.GOLDEN_CARROT)) {
					CivMessage.sendError(event.getPlayer(),
							CivSettings.localize.localizedString("itemUse_errorNoHorseBreeding"));
					event.setCancelled(true);
					return;
				}
				break;
			...
			if (denyBreeding) {
				...
				if (pasture == null) {
					CivMessage.sendError(event.getPlayer(),
							CivSettings.localize.localizedString("itemUse_errorNoWildBreeding"));
					event.setCancelled(true);
				} else {
					int loveTicks;
		...
		if (!tc.perms.hasPermission(PlotPermissions.Type.INTERACT, resident)) {
			event.setCancelled(true);
			CivMessage.sendErrorNoRepeat(event.getPlayer(),
					CivSettings.localize.localizedString("itemUse_errorPaintingOrFrame"));
		}
...
//			}
			if (event.getRemover() instanceof Player) {
				CivMessage.sendError((Player) event.getRemover(),
						CivSettings.localize.localizedString("blockBreak_errorItemFrame"));
			}
			event.setCancelled(true);
			return;
		}
		...
		if (event.getRemover() instanceof Player) {
			...
			if (!tc.perms.hasPermission(PlotPermissions.Type.DESTROY, resident)) {
				...
				CivMessage.sendErrorNoRepeat(player,
						CivSettings.localize.localizedString("blockBreak_errorFramePermission"));
			}
		}
...
//		}
	...
	public boolean allowPistonAction(Location loc) {
		...
		/*
		 * If we're next to an attached protected item frame. Disallow we cannot break
		 * protected item frames.
		 * 
	...
	public void onBlockPistonExtendEvent(BlockPistonExtendEvent event) {
		...
		/*
		 * UGH. If we extend into 'air' it doesnt count them as blocks... we need to
		 * check air to prevent breaking of item frames...
		 */
...
//		}
		...
		for (LivingEntity entity : event.getAffectedEntities()) {
			if (entity instanceof Player) {
				...
				case ALLOWED:
					continue;
				case NOT_AT_WAR:
					CivMessage.send(attacker, CivColor.Rose
							+ CivSettings.localize.localizedString("var_itemUse_potionError1", defender.getName()));
					event.setCancelled(true);
					return;
				case NEUTRAL_IN_WARZONE:
					CivMessage.send(attacker, CivColor.Rose
							+ CivSettings.localize.localizedString("var_itemUse_potionError2", defender.getName()));
					event.setCancelled(true);
					return;
				case NON_PVP_ZONE:
					CivMessage.send(attacker, CivColor.Rose
							+ CivSettings.localize.localizedString("var_itemUse_potionError3", defender.getName()));
					event.setCancelled(true);
					return;
				}
			}
	...
	private enum PVPDenyReason {
		ALLOWED, NON_PVP_ZONE, NOT_AT_WAR, NEUTRAL_IN_WARZONE
	}
	...
	private PVPDenyReason playersCanPVPHere(Player attacker, Player defender, TownChunk tc) {
		...
		if (War.isWarTime()) {
			if (tc.getTown().getCiv().getDiplomacyManager().isAtWar()) {
				/*
				 * The defender is neutral if he is not in a town/civ, or not in his own civ AND
				 * not 'at war' with the attacker.
				 */
				if (!defenderResident.hasTown()
						|| (!defenderResident.getTown().getCiv().getDiplomacyManager().atWarWith(tc.getTown().getCiv())
								&& defenderResident.getTown().getCiv() != tc.getTown().getCiv())) {
					/* Allow neutral players to be hurt, but not hurt them back. */
					return PVPDenyReason.ALLOWED;
				} else if (!attackerResident.hasTown()
						|| (!attackerResident.getTown().getCiv().getDiplomacyManager().atWarWith(tc.getTown().getCiv())
		...
		if (defenderResident != null && defenderResident.hasTown()) {
			...
			/*
			 * If defenders are at war with attackers allow PVP. Location doesnt matter.
			 * Allies should be able to help defend each other regardless of where they are
			 * currently located.
			 */
