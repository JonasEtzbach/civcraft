...
//			}
	...
	public void onItemPickup(PlayerPickupItemEvent event) {
		...
		if (ItemManager.getId(event.getItem().getItemStack()) == ItemManager.getId(Material.SLIME_BALL)) {
			...
			if (craftMat == null) {
				...
				&& ItemManager.getData(event.getItem().getItemStack()) == ItemManager
						.getData(ItemManager.getMaterialData(CivData.FISH_RAW, CivData.CLOWNFISH))) {
			LoreCraftableMaterial craftMat = LoreCraftableMaterial.getCraftMaterial(event.getItem().getItemStack());
			if (craftMat == null) {
				...
				&& ItemManager.getData(event.getItem().getItemStack()) == ItemManager
						.getData(ItemManager.getMaterialData(CivData.FISH_RAW, CivData.PUFFERFISH))) {
			LoreCraftableMaterial craftMat = LoreCraftableMaterial.getCraftMaterial(event.getItem().getItemStack());
			if (craftMat == null) {
	...
	public void convertLegacyItem(InventoryClickEvent event) {
		boolean currentEmpty = (event.getCurrentItem() == null)
				|| (ItemManager.getId(event.getCurrentItem()) == CivData.AIR);
		...
		if (ItemManager.getId(event.getCurrentItem()) == ItemManager.getId(Material.RAW_FISH)
				&& ItemManager.getData(event.getCurrentItem()) == ItemManager
						.getData(ItemManager.getMaterialData(CivData.FISH_RAW, CivData.CLOWNFISH))) {
			LoreCraftableMaterial craftMat = LoreCraftableMaterial.getCraftMaterial(event.getCurrentItem());
			if (craftMat == null) {
				/* Found a vanilla slime ball. */
	...
	public void OnInventoryClick(InventoryClickEvent event) {
		...
		if (event.getRawSlot() < 0) {
			...
			// CivLog.debug("GET RAW WAS NEGATIVE, cancel this event it should be
			// invalid.");
		...
		if (event.isShiftClick()) {
			...
			if (current != null) {
				// if (is.isCustomItem() && (is.getMaterial() instanceof
				// CustomMaterialExtended)) {
				// Calling onInvShiftClick Event.
				// ((CustomMaterialExtended)is.getMaterial()).onInvShiftClick(event, clickedInv,
				// otherInv, is.getItem());
...
//	/* Prevent books from being inside an inventory. */
	...
	public void OnInventoryClickEvent(InventoryClickEvent event) {
		...
		// removeUnwantedVanillaItems((Player)event.getWhoClicked(),
