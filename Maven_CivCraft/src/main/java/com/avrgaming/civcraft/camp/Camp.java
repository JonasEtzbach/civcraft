...
public class Camp extends Buildable {
	...
	public void loadSettings() {
		try {
			...
			sifter.addSiftItem(ItemManager.getId(Material.COBBLESTONE), (short) 0, gold_nugget_chance,
					ItemManager.getId(Material.GOLD_NUGGET), (short) 0, 1);
			sifter.addSiftItem(ItemManager.getId(Material.COBBLESTONE), (short) 0, iron_ignot_chance,
					ItemManager.getId(Material.IRON_INGOT), (short) 0, 1);
			sifter.addSiftItem(ItemManager.getId(Material.COBBLESTONE), (short) 0, 1.0,
					ItemManager.getId(Material.GRAVEL), (short) 0, 1);
	...
	public static void init() throws SQLException {
		if (!SQL.hasTable(TABLE_NAME)) {
			String table_create = "CREATE TABLE " + SQL.tb_prefix + TABLE_NAME + " ("
					+ "`id` int(11) unsigned NOT NULL auto_increment," + "`name` VARCHAR(64) NOT NULL,"
					+ "`owner_name` mediumtext NOT NULL," + "`firepoints` int(11) DEFAULT 0," + "`next_raid_date` long,"
					+ "`corner` mediumtext," + "`upgrades` mediumtext," + "`template_name` mediumtext,"
					+ "PRIMARY KEY (`id`)" + ")";
	...
	public void buildCamp(Player player, Location center) throws CivException {
		...
		if (resident.desiredTemplate == null) {
			try {
				...
				String templatePath = Template.getTemplateFilePath(templateFile, Template.getDirection(center),
	...
	private void processCommandSigns(Template tpl, BlockCoord corner) {
		for (BlockCoord relativeCoord : tpl.commandBlockRelativeLocations) {
			...
			BlockCoord absCoord = new BlockCoord(
					corner.getBlock().getRelative(relativeCoord.getX(), relativeCoord.getY(), relativeCoord.getZ()));
	...
	public void processFirepoints() {
		...
		if (mInv.contains(null, CivData.COAL, (short) 0, coal_per_firepoint)) {
			...
			CivMessage.sendCamp(this,
					CivColor.Yellow + CivSettings.localize.localizedString("var_camp_campfireDown", this.firepoints));

			double percentLeft = (double) this.firepoints / (double) this.maxFirePoints;
			if (percentLeft < 0.3) {
				CivMessage.sendCamp(this, CivColor.Yellow + ChatColor.BOLD
						+ CivSettings.localize.localizedString("camp_campfire30percent"));
			}
	...
	public void processLonghouse() {
		...
		case LEVELDOWN:
			CivMessage.sendCamp(this,
					CivColor.LightGreen + CivSettings.localize.localizedString("var_camp_yourLonghouseDown",
							(CivColor.Rose + CivSettings.localize.localizedString("camp_longhouseStavedAndLeveledDown")
									+ CivColor.LightGreen),
							CivSettings.CURRENCY_NAME));
			return;
		case STAGNATE:
		...
		CivMessage.sendCamp(this, CivColor.LightGreen + CivSettings.localize.localizedString("var_camp_yourLonghouse",
				stateMessage, total_coins, CivSettings.CURRENCY_NAME));
	}
			...
			for (int y = 0; y < tpl.size_y; y++) {
				for (int z = 0; z < tpl.size_z; z++) {
					...
					if (tpl.blocks[x][y][z].specialType == Type.LITERAL) {
						...
						tpl.commandBlockRelativeLocations
								.add(new BlockCoord(cornerBlock.getWorld().getName(), x, y, z));
						continue;
					}
	...
	private void bindCampBlocks() {
		...
		// on the corner
		// location and the template's size. We need to verify that each block is a part
		// of the template.
	...
	public void onControlBlockHit(ControlPoint cp, World world, Player player) {
		...
		CivMessage.send(player, CivColor.LightGray + CivSettings.localize.localizedString("camp_hitControlBlock") + "("
				+ cp.getHitpoints() + " / " + cp.getMaxHitpoints() + ")");
		CivMessage.sendCamp(this,
				CivColor.Yellow + CivSettings.localize.localizedString("camp_controlBlockUnderAttack"));
	}
	...
	public void onControlBlockDestroy(ControlPoint cp, World world, Player player) {
		...
		if (allDestroyed) {
			...
			CivMessage.sendCamp(this,
					CivColor.Rose + CivSettings.localize.localizedString("camp_controlBlockDestroyed"));
		}
			...
			if (now.after(getNextRaidDate())) {
				if (!cp.isDestroyed()) {
					...
					CivMessage.send(player,
							CivColor.Rose + CivSettings.localize.localizedString("camp_controlBlockAlreadyDestroyed"));
				}
			} else {
				SimpleDateFormat sdf = new SimpleDateFormat("M/dd h:mm:ss a z");
				CivMessage.send(player, CivColor.Rose + CivSettings.localize.localizedString("camp_protectedUntil")
						+ " " + sdf.format(getNextRaidDate()));
			}
