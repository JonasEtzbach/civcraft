...
public class CannonProjectile {
	...
	public void onHit() {
								...
								if (!structuresHit.contains(sb.getOwner())) {
									...
									if (sb.getOwner() instanceof TownHall) {
										...
										if (!sb.getCiv().getDiplomacyManager().atWarWith(whoFired.getCiv())) {
											if (player != null) {
												CivMessage.sendError(player, CivSettings.localize
														.localizedString("cannonProjectile_ErrorNotAtWar"));
												return;
											}
										}
										...
										sb.getOwner().onDamage(cannon.getDamage(), b.getWorld(), player, sb.getCoord(),
												sb);
										CivMessage.sendCiv(sb.getCiv(), CivColor.Yellow
												+ CivSettings.localize.localizedString("cannonProjectile_hitAnnounce",
									...
									CivMessage.sendCiv(whoFired.getCiv(), CivColor.LightGreen
											+ CivSettings.localize.localizedString("var_cannonProjectile_hitSuccess",
													sb.getOwner().getTown().getName(), sb.getOwner().getDisplayName())
											+ " (" + sb.getOwner().getHitpoints() + "/"
											+ sb.getOwner().getMaxHitPoints() + ")");
								}
							} else {
		...
		for (Entity e : players) {
			...
			if (player.isDead()) {
				CivMessage.global(CivColor.LightGray + CivSettings.localize
						.localizedString("var_cannonProjectile_userKilled", player.getName(), whoFired.getName()));
			}
		}
	}
	...
	private void launchExplodeFirework(Location loc) {
		...
		if (rand1 > 90) {
			FireworkEffect fe = FireworkEffect.builder().withColor(Color.ORANGE).withColor(Color.YELLOW).flicker(true)
