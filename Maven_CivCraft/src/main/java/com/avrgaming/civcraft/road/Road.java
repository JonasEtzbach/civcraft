...
public class Road extends Structure {
	...
	/*
	 * A road is a special type of structure that is built like a Wall but is
	 * layered on the ground. The road allows faster movement speeds. Road blocks
	 * are attackable for a 2 hour window based on when they are built.
	 */
	...
	public void processUndo() throws CivException {
		...
		CivMessage.sendTown(this.getTown(), CivColor.Yellow
				+ CivSettings.localize.localizedString("var_road_undoComplete", totalCost, CivSettings.CURRENCY_NAME));
	...
	public void buildPlayerPreview(Player player, Location centerLoc) throws CivException, IOException {
		 ...
		 */
		MarkerPlacementManager.addToPlacementMode(player, this,
				CivSettings.localize.localizedString("road_startPlacement"));
	}
...
//		MarkerPlacementManager.addToPlacementMode(player, this, "Road Marker");
	...
	public void onMarkerPlacement(Player player, Location next, ArrayList<Location> locs) throws CivException {
		...
		for (SimpleBlock sb : simpleBlocks.values()) {
			...
			if (!validateBlockLocation(bcoord)) {
				/*
				 * This block cannot be placed here, but it will NOT stop the construction of
				 * the road.
				 */
				removed.add(sb);
			}
		...
		if (!this.getTown().getTreasury().hasEnough(totalCost)) {
			throw new CivException(
					CivSettings.localize.localizedString("var_road_tooPoor", totalCost, CivSettings.CURRENCY_NAME));
		}
		...
		for (SimpleBlock sb : simpleBlocks.values()) {
			...
			/* Save old block data. */
			// this.oldBlockData.put(new BlockCoord(bcoord), new
			// SimpleBlock(bcoord.getBlock()));
			addRoadBlock(bcoord);
		...
		CivMessage.sendTown(this.getTown(),
				CivColor.LightGreen + CivSettings.localize.localizedString("var_road_success",
						CivColor.Yellow + totalCost + CivColor.LightGreen, CivSettings.CURRENCY_NAME));
	...
	/*
	 * We're going to try something tricky here. In order to make building roads not
	 * annoying we can't have our own structures getting in the way. However if we
	 * dont "halt" for structures then an enemy can easily use the road to "slice"
	 * through our structures and grief us.
	 * 
	 ...
	 * potential griefing situation and we'll cancel the entire road. If it's one of
	 * our structure blocks, we'll assume the player knows what they are doing and
	 * allow the constructio to continue, minus the offending blocks.
	 * 
	 */
	...
	private boolean validateBlockLocation(BlockCoord startCoord) throws CivException {
		...
		for (int i = 0; i < Road.HEIGHT; i++) {
			...
			if (ItemManager.getId(bcoord.getBlock()) == CivData.CHEST) {
				throw new CivException(
						CivSettings.localize.localizedString("var_road_validate_wouldDestroyChest", bcoord.toString()));
			}

			if (CivGlobal.getProtectedBlock(bcoord) != null) {
				throw new CivException(
						CivSettings.localize.localizedString("var_road_validate_protectedBlock", bcoord.toString()));
			}
			...
			if (structBlock != null) {
				...
				if (structBlock.getCiv() != this.getCiv()) {
					throw new CivException(CivSettings.localize.localizedString("var_road_validate_structure",
							structBlock.getCiv().getName(),
			...
			if (rb != null) {
				...
				if (rb.getRoad().getCiv() != this.getCiv()) {
					throw new CivException(CivSettings.localize.localizedString("var_road_validate_roadInWay",
							rb.getRoad().getCiv().getName(),
	...
	private int buildRoadSegment(Player player, Location locFirst, Location locSecond, int blockCount,
		...
		Vector dir = new Vector(locFirst.getX() - locSecond.getX(), locFirst.getY() - locSecond.getY(),
...
//			}
	...
	private void getHorizontalSegment(Player player, Location loc, HashMap<String, SimpleBlock> simpleBlocks) {
		...
		/*
		 * Go through each location here and draw a circle around it the circles will
		 * represent the line's width.
		 */
		Road.getCircle(tmp.getBlockX(), tmp.getBlockY(), tmp.getBlockZ(), tmp.getWorld().getName(), Road.WIDTH,
				simpleBlocks);
	}

	public static void getCircle(int blockX, int blockY, int blockZ, String world, int radius,
	...
	private static void plotFourPoints(int cx, int cz, int x, int z, int baseY, String world,
			HashMap<String, SimpleBlock> simpleBlocks) {
...
//		setPixel(cx - x, baseY, cz - z, world, simpleBlocks);
	...
	private static void horizontalLine(int x, int y, int z, int size, String world,
			HashMap<String, SimpleBlock> simpleBlocks) {
	...
	public void addBlocksAbove(RoadBlock rb) {
		...
		/*
		 * Add blocks above the road, but do not save them in the Road structure. This
		 * prevents them from being saved but still allows us to look them up via the
		 * bcoord hash so that we can prevent non-owners from placing blocks on the
		 * road.
		 */
		for (int i = 1; i < Road.HEIGHT; i++) {
	...
	public void deleteRoadBlock(RoadBlock roadBlock) {
		try {
			...
			if (this.roadBlocks.size() == 0) {
				...
				CivMessage.sendTown(this.getTown(),
						CivSettings.localize.localizedString("var_road_destroyed", this.getCorner()));
				this.delete();
			}
		} catch (SQLException e) {
			e.printStackTrace();
	...
	public void onDamage(int amount, World world, Player player, BlockCoord coord, BuildableDamageBlock hit) {
		...
		if (hit.getOwner().isDestroyed()) {
			CivMessage.sendError(player,
					CivSettings.localize.localizedString("var_road_alreadyDestroyed", hit.getOwner().getDisplayName()));
			return;
		}

		if (!hit.getOwner().isComplete() && !(hit.getOwner() instanceof Wonder)) {
			CivMessage.sendError(player, CivSettings.localize.localizedString("var_road_underConstruction",
	...
	public void onDestroy() {
		...
		CivMessage.global(CivSettings.localize.localizedString("var_road_destroySuccess", this.getDisplayName(),
